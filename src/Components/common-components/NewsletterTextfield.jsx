...
const sendMail = formData => {
    ...
    emailjs.send(serviceID, templateID, templateParams, publicKey)
      .then(response=> {
        ...
        //no-replay mail
        emailjs.send(import.meta.env.VITE_APP_THANK_YOU_MAIL_SERVICE_ID, import.meta.env.VITE_APP_THANK_YOU_MAIL_NEWSLETTER_TEMPLATE_ID, {email:formData.email}, import.meta.env.VITE_APP_THANK_YOU_MAIL_PUBLIC_KEY)
                .then(response=> {
                  if(response.status===200){
...
const onSubmit = (formData, { resetForm, setSubmitting, setFieldError })=> {
    const payload = {
      ...
      axios
      .post(`${import.meta.env.VITE_APP_API_URL}api/save-newsletter-users`, payload)
      .then(res=> {
...
let formik = useFormik({
  ...
  return (
    ...
    <div
    className="flex items-center dark:bg-[#50545a] px-3 rounded-full bg-white">
                  <input
                  ...
                  >
                    {formik.isSubmitting ? 
                     <svg aria-hidden="true" className="w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-secondary" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
