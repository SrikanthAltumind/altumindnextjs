...
useEffect(()=> {
  ...
  return (
    <div>
        <Slider {...settings} className=' dark:text-white bg-lightBlue dark:bg-gradient-to-b from-primary to-darkTheme font-raleway'>
            {bannerData.map(item=> 
                <div key={item.id}>
                <div className='min-h-[calc(100vh-40px)] sm:min-h-[calc(100vh-300px)] sm:flex justify-center  md:px-5 max-sm:pt-10'>
                    <div className='basis-[50%] flex items-center justify-center'>
                        <img className='max-w-[300px] md:max-w-[400px] lg:max-w-[500px] object-contain' src={flyingRocket} alt="rocket"/>                    
                    </div>
                    <div className='basis-[50%] px-5 md:px-0 lg:pl-10 flex flex-col justify-center items-start gap-2 lg:gap-7 pb-2'>
                        ...
                        <p className=' text-primary dark:text-white leading-[1.2]'>
                            {item.heading1}
                            {item?.dynamicWords &&
                                <span className='border-r-2 border-primary dark:border-white pr-1 custom-gradient-text text-3xl xl:text-5xl'>
                                &nbsp;{word.slice(0,charIndex)}
                            </span>
                            }
                        </p>
                            <p className='py-2 custom-gradient-text text-3xl xl:text-5xl'>{item.heading2}</p>
                        </div>
