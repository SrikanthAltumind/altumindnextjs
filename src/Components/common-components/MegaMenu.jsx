...
const MegaMenu = ({item}) => {
  ...
  return (
    <>
  <div className="absolute dark:text-black w-[90%] right-20 z-10 top-full">
        ...
        <div className="flex text-[13px] font-medium justify-between w-full h-full">
          ...
          <div className="border-r basis-[50%] w-full h-full">
            <ul className="text-black font-montserrat">
            {
              ...
              <Link to={child?.path}  key={index}><li onMouseOver={() => setSubMenu(child)} className={`${subMenu?.innerMenu === child.innerMenu ? 'bg-[#DE5346] text-white' : '' } rounded-md flex gap-2 items-center  p-2 m-2`}>{subMenu?.innerMenu === child.innerMenu ? <span className="bg-white p-1 rounded-full"></span> : ''}{child.innerMenu}
              </li></Link>
              ))
            }
            </ul>
