        ...
        <div className="font-raleway font-semibold text-center text-primary dark:text-white">
          ...
          >
            {data?.data?.slice(-3)?.map((card) => (
              <div
                key={card?.id}
                className="w-[250px] sm:w-[31%] max-w-[280px] space-y-3 text-start"
              >
                ...
                <a href={`/blog/${card?.id}`} className="text-sm group px-2 flex items-center cursor-pointer gap-1 max-w-max">
