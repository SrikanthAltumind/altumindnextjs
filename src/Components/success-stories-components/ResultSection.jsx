...
const ResultSection = () => {
  ...
  const fetchStatsData = ()=> {
      axios.get(`${import.meta.env.VITE_APP_API_URL}${statsAPIUrl}`)
          .then(response=> {
...
if (loading) {
  ...
  return (
    <div className='py-10 space-y-10 w-[90%] max-w-[950px] mx-auto dark:text-white font-raleway' >
        <p className='custom-gradient-text text-center'>
            {headingData?.title}
            <br/>
            {headingData?.heading}
        </p>
              ...
              {statsData?.map(stat=> (
                <div key={stat.id} className='flex flex-col gap-y-2 md:gap-y-4 text-center '>
                   ...
                   <p className='text-3xl text-secondary font-bold'>
                    <Counter value={Number(stat?.attributes?.percent?.replace(/[%x]/g, ''))} countSpeed={80}/>
                    <span>{stat?.attributes?.percent?.includes('x')? 'x' : '%'}</span>
                  </p>
                  <div className='font-montserrat space-y-1'>
                    <p className='font-medium dark:font-normal text-sm '>{stat?.attributes?.description}</p>
                    {/* {stat?.description && 
