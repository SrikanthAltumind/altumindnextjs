...
const LeaderBoard = () => {
  ...
  const fetchLeadersData = ()=> {
    axios.get(`${import.meta.env.VITE_APP_API_URL}api/leadership-meet-our-leader-boards?populate=*`)
    .then(response=> {
  ...
  const fetchMessageData = ()=> {
    axios.get(`${import.meta.env.VITE_APP_API_URL}api/leadership-message`)
    .then(response=> {
  ...
  return (
    <div className='w-[90%] max-w-[950px] mx-auto dark:text-white space-y-10'>
      ...
      <div className="font-raleway text-center dark:text-white mb-32">
        ...
        
          {
            leaders?.map((leader) => (
              ...
              <div className="relative dark:bg-DarkBackground  rounded-xl w-[320px] pt-14 py-7 px-6 shadow-custom-shadow space-y-7">
                  <img className="rounded-2xl absolute mx-auto left-[80px] -top-24" src={leader?.attributes?.image?.data?.attributes?.url} alt={leader?.attributes?.name} height={160} width={160} />
                  <div className="font-montserrat space-y-1">
                      <p className="font-bold ">{leader?.attributes?.name}</p>
                      <p className="text-sm font-medium">{leader?.attributes?.designation}</p>
                  </div>
                  <p className="text-[13px] font-medium font-montserrat leading-relaxed">{leader?.attributes?.intro}</p>
        ...
        <span className='text-secondary text-6xl absolute left-0 '>&ldquo;</span>
            <p className='text-lg font-semibold dark:font-medium font-raleway'> {message?.title}</p>
