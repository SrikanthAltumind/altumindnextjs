      ...
      <div className="max-w-[1000px] mx-auto">
        {location.pathname === "/services/experience-design" && (
          <div className="text-center space-y-3 mb-10">
            ...
            <p className=" font-extrabold text-3xl  lg:text-4xl text-transparent bg-clip-text bg-gradient-to-r from-gradientBlue  via-gradientGreen to-gradientBlue">
              Your Partner in Experience-Driven Growth
            </p>
          </div>
        )}
        <div className="flex justify-center sm:flex-row  flex-wrap lg:flex-nowrap flex-col gap-5  ">
          {data?.map((item) => (
            <div
              key={item.id}
              className="max-w-[300px] flex justify-between flex-col text-white  lg:h-auto md:h-[300px] h-[350px] space-y-2 p-5 rounded-xl mx-auto"
              style={{
                backgroundColor:
                  theme === "dark" && item.id === 1
                    ? "#5EC7A7"
                    : item?.attributes?.hexvalue,
              }}
            >
              <div className="space-y-2">
                <p className="font-semibold">{item?.attributes?.title}</p>
                <p className="text-[13px] font-light font-montserrat">
                  {item?.attributes?.description}
                </p>
              </div>
              <Link to={item?.attributes?.url} className="w-fit">
                <p className="text-[13px] flex gap-2 items-center w-fit group font-semibold">
                  {item?.attributes?.cta || item?.attributes?.CTA}
                  <svg
