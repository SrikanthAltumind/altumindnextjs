...
const ContactMenu = () => {
  const [selectedForm, setSelectedForm] = useState(formTabs[0]);
  const [leftSection, setLeftSection] = useState();

  useEffect(() => {
    setLeftSection(contentSection[selectedForm?.id]);
  }, [selectedForm]);
      ...
      <div className="w-[90%] max-w-[450px] md:max-w-[700px] mx-auto flex justify-between dark:text-white">
        {formTabs?.map((tab) => (
          <button
            ...
            className={`text-xs sm:text-sm font-semibold border-b-[3.5px] py-1 px-1 sm:px-4 hover:border-secondary ${
              selectedForm.id === tab.id
                ? "border-secondary"
                : "border-transparent"
            }`}
          >
      ...
      <div className="mt-7">
        ...
        >
          ...
          <div className="basis-[30%] shrink-0 p-5 lg:p-10 pb-2 font-raleway bg-LightBlue dark:bg-DarkBackground flex flex-col items-center md:items-start gap-4">
            <img
              src={leftSection?.imgUrl}
              className="w-36 h-36 lg:w-52 lg:h-52 object-cover bg-merald-200"
            />
            <p className="custom-gradient-text md:mx-0 max-md:text-center text-2xl lg:text-3xl capitalize">
              {leftSection?.caption}
            </p>
