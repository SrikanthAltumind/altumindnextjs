        ...
        <div className='w-[90%] max-w-[450px] md:max-w-[700px] mx-auto flex justify-between dark:text-white'>
            {formTabs?.map(tab=> (
                <button
                    ...
                    className={`text-xs sm:text-sm font-semibold border-b-[3.5px] py-1 px-1 sm:px-4 hover:border-secondary ${selectedForm.id===tab.id ? 'border-secondary' : 'border-transparent'}`}>
                {tab.name}</button>
            ))}
        </div>
        {/* Selected Form UI */}
