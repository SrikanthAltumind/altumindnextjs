  ...
  const onSubmit = (formData, { resetForm }) => {
    const payload = {
      data: {
        ...
        littleAboutYou: formData.littleAboutYou.trim(),
      },
    };
  ...
  return (
    <div className={`w-[90%] md:w-[95%] max-w-[1200px] mx-auto dark:text-white flex flex-col md:flex-row shadow-custom-shadow`}>
      <div className="basis-[35%] shrink-0 lg:basis-[30%] p-5 lg:p-10 pb-2 font-raleway bg-LightBlue dark:bg-DarkBackground flex flex-col items-center md:items-start gap-4">
        <img src={contentSection?.imgUrl} className="w-36 h-36 lg:w-52 lg:h-52 object-cover bg-merald-200"/>
          <p className="custom-gradient-text md:mx-0  max-md:text-center text-2xl lg:text-3xl">{contentSection?.title}</p>
          <p className="uppercase font-semibold text-xs sm:text-sm">{contentSection?.caption}</p>
          <p className="max-md:text-center text-xs sm:text-sm lg:text-base max-md:max-w-[450px] font-montserrat font-medium leading-5">
      ...
      >
        ...
        <div className="relative flex flex-col w-full">
          <textarea
            ...
            id="littleAboutYou"
            {...formik.getFieldProps("littleAboutYou")}
          />
          <label
            htmlFor="littleAboutYou"
            className={`w-full absolute cursor-text top-1 peer-focus:text-xs ${
              formik.values.littleAboutYou ? "text-xs -translate-y-6" : "text-sm lg:text-base"
            }  peer-focus:-translate-y-6 transition-all  duration-300 peer-focus:text-primary font-medium peer-focus:dark:text-blue-400`}
          >
            A little about You:
          </label>
          <div className="before:content-[''] before:h-[1px] before:w-full before:bg-primary before:absolute before:bottom-0 scale-0 peer-focus:scale-100 transition-all duration-300 before:dark:bg-blue-400 ease-linear"></div>
          {formik.touched.littleAboutYou && formik.errors.littleAboutYou && (
            <p className="absolute -bottom-[18px] text-secondary text-xs">
              *{formik.errors.littleAboutYou}
            </p>
          )}
        </div>
      {/* Upload your CV */}
        <div className="w-full">
            <label className="block mb-3 font-medium">Upload your CV<span className="text-red-500">*</span></label>
            <div className='border border-gray-300 dark:border-gray-600  rounded-lg'>
              <input onChange={e=> {setFile(e.target.files)}}
                type="File" accept='docs/*' id="fileUpload" name="fileUpload" placeholder="Upload here"  
                className='hidden'
